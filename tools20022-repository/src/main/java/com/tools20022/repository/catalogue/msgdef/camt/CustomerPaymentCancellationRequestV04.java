...
import com.tools20022.repository.catalogue.msgdef.camt.CustomerPaymentCancellationRequestV03;
...
import com.tools20022.repository.dict.msg.UnderlyingTransaction11;
 ...
 * ReopenCaseIndication is set to 'Yes' or 'true'. The Customer Payment
 * Cancellation Request message has the following main characteristics: The case
 * creator assigns a unique case identification and the reason code for the
 * cancellation request. This information will be passed unchanged to all
 ...
 * case management environment where the case identification is not relevant.
 * Moreover, the case identification may be present at different levels: - One
 * unique case is defined per cancellation request message: If multiple
 ...
 * message. - One case per original group, payment information or transaction
 * present in the cancellation request: For each group, payment information
 * block or transaction within the payment information, a unique case has been
 * assigned. This means, when a payment instruction has already been forwarded
 * by the case assignee, the cancellation request may be forwarded to next party
 * in the payment chain, with the unique case assigned to the transaction. When
 * the group can only be cancelled partially, new cancellation requests need
 * however to be issued for the individual transactions within the group for
 * which the cancellation request has not been successful. In response to this
 ...
 * Investigation message. - No case used in cancellation request message: The
 * cancellation of a payment instruction can be initiated by either the
 * debtor/creditor or any subsequent agent in the payment instruction processing
 * chain.
 */
@PreviousVersion(CustomerPaymentCancellationRequestV03.class)
@NextVersion(CustomerPaymentCancellationRequestV05.class)
public class CustomerPaymentCancellationRequestV04 {
	...
	private UnderlyingTransaction11 Underlying;
	...
	public UnderlyingTransaction11 getUnderlying() {
		return Underlying;
	}

	public void setUnderlying(UnderlyingTransaction11 Underlying) {

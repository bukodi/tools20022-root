...
import com.tools20022.repository.catalogue.msgdef.setr.RedemptionOrderCancellationRequestV03;
...
/**
 * Scope The RedemptionOrderCancellationRequest message is sent by an
 ...
 * request the cancellation of a previously sent RedemptionOrder. Usage The
 * RedemptionOrderCancellationRequest message is used to message is used to
 * request the cancellation of one or more individual orders. There is no
 * amendment, but a cancellation and re-instruct policy. To request the
 * cancellation of one or more individual orders, the order reference of each
 * individual order listed in the original RedemptionOrder message in the order
 * reference element. The message identification of the RedemptionOrder message
 * which contains the individual orders to be cancelled may also be quoted in
 * PreviousReference but this is not recommended. The deadline and acceptance of
 * a cancellation request is subject to a service level agreement (SLA). This
 * cancellation message is a cancellation request. There is no automatic
 * acceptance of the cancellation. The rejection or acceptance of a
 * RedemptionOrderCancellationRequest is made using an
 * OrderCancellationStatusReport message.
 */
@PreviousVersion(RedemptionOrderCancellationRequestV03.class)

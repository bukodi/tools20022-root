package com.tools20022.repository.dict.codeset;


import com.tools20022.metamodel.MMCodeSet;
import java.util.concurrent.atomic.AtomicReference;
import com.tools20022.metamodel.MMCode;
import com.tools20022.repository.dict.codeset.RTCashClearingSystemCode;
import java.util.Arrays;
import com.tools20022.repository.GeneratedRepository;/**
 * Specification of a pre-agreed offering between clearing agents or the channel through which the payment instruction is to be processed.
 */
public class RTCashClearingSystemCode extends MMCodeSet {

	private final static AtomicReference<RTCashClearingSystemCode> repoTypeRef = new AtomicReference<>();
	/**
	 * Scheme code for EBA Euro1/Step1.
	 */
	public final MMCode codeEBAEuro1Step1 = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "ABE";
			super.name = "EBAEuro1Step1";
			super.definition = "Scheme code for EBA Euro1/Step1.";
		}
	};
	/**
	 * Scheme code for AT (Austria) - Austrian RTGS (ARTIS).
	 */
	public final MMCode codeAustrian = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "ART";
			super.name = "Austrian";
			super.definition = "Scheme code for AT (Austria) - Austrian RTGS (ARTIS).";
		}
	};
	/**
	 * Scheme code for NZ (New Zealand) - New Zealand Assured Value Payments.
	 */
	public final MMCode codeNewZealand = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "AVP";
			super.name = "NewZealand";
			super.definition = "Scheme code for NZ (New Zealand) - New Zealand Assured Value Payments.";
		}
	};
	/**
	 * Scheme code for AZ (Azerbaijan) - Azerbaijan Interbank Payment System (AZIPS).
	 */
	public final MMCode codeAzerbaijan = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "AZM";
			super.name = "Azerbaijan";
			super.definition = "Scheme code for AZ (Azerbaijan) - Azerbaijan Interbank Payment System (AZIPS).";
		}
	};
	/**
	 * Scheme code for BA (Bosnia and Herzegovina).
	 */
	public final MMCode codeBosniaHerzegovina = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "BAP";
			super.name = "BosniaHerzegovina";
			super.definition = "Scheme code for BA (Bosnia and Herzegovina).";
		}
	};
	/**
	 * Scheme code for BE (Belgium) - Belgium RTGS (ELLIPS).
	 */
	public final MMCode codeBelgium = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "BEL";
			super.name = "Belgium";
			super.definition = "Scheme code for BE (Belgium) - Belgium RTGS (ELLIPS).";
		}
	};
	/**
	 * Scheme code for FI (Finland) - RTGS (BOF).
	 */
	public final MMCode codeFinland = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "BOF";
			super.name = "Finland";
			super.definition = "Scheme code for FI (Finland) - RTGS (BOF).";
		}
	};
	/**
	 * Scheme code for IT (Italy) - Italy RTGS (BIREL).
	 */
	public final MMCode codeItaly = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "BRL";
			super.name = "Italy";
			super.definition = "Scheme code for IT (Italy) - Italy RTGS (BIREL).";
		}
	};
	/**
	 * Scheme code for CA (Canada) - Canadian Large Value Transfer System (LVTS)
	 */
	public final MMCode codeCanada = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "CAD";
			super.name = "Canada";
			super.definition = "Scheme code for CA (Canada) - Canadian Large Value Transfer System (LVTS)";
		}
	};
	/**
	 * Scheme code for ES (Spain).
	 */
	public final MMCode codeSpainCAM = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "CAM";
			super.name = "SpainCAM";
			super.definition = "Scheme code for ES (Spain).";
		}
	};
	/**
	 * Scheme code for IE (Ireland) - Irish RTGS (IRIS).
	 */
	public final MMCode codeIreland = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "CBJ";
			super.name = "Ireland";
			super.definition = "Scheme code for IE (Ireland) - Irish RTGS (IRIS).";
		}
	};
	/**
	 * Scheme code for GB (UK) - British Euro RTGS (CHAPS).
	 */
	public final MMCode codeUnitedKingdom = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "CHP";
			super.name = "UnitedKingdom";
			super.definition = "Scheme code for GB (UK) - British Euro RTGS (CHAPS).";
		}
	};
	/**
	 * Scheme code for DK (Denmark) - Danish Euro RTGS (KRONOS)
	 */
	public final MMCode codeDenmark = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "DKC";
			super.name = "Denmark";
			super.definition = "Scheme code for DK (Denmark) - Danish Euro RTGS (KRONOS)";
		}
	};
	/**
	 * Scheme code for DE (Germany).
	 */
	public final MMCode codeGermanyRTGSPlus = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "RTP";
			super.name = "GermanyRTGSPlus";
			super.definition = "Scheme code for DE (Germany).";
		}
	};
	/**
	 * Scheme code for EBA Euro1.
	 */
	public final MMCode codeEBAEuro1 = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "EBA";
			super.name = "EBAEuro1";
			super.definition = "Scheme code for EBA Euro1.";
		}
	};
	/**
	 * Scheme code for DE (Germany).
	 */
	public final MMCode codeGermanyELS = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "ELS";
			super.name = "GermanyELS";
			super.definition = "Scheme code for DE (Germany).";
		}
	};
	/**
	 * Scheme code for EBA step 1 (members).
	 */
	public final MMCode codeEBAStep1 = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "ERP";
			super.name = "EBAStep1";
			super.definition = "Scheme code for EBA step 1 (members).";
		}
	};
	/**
	 * Scheme code for EBA step 2.
	 */
	public final MMCode codeEBAStep2 = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "XCT";
			super.name = "EBAStep2";
			super.definition = "Scheme code for EBA step 2.";
		}
	};
	/**
	 * Scheme code for HR (Croatia) - HSVP.
	 */
	public final MMCode codeCroatia = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "HRK";
			super.name = "Croatia";
			super.definition = "Scheme code for HR (Croatia) - HSVP.";
		}
	};
	/**
	 * Scheme code for GR (Greece) - Greek RTGS (HERMES).
	 */
	public final MMCode codeGreece = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "HRM";
			super.name = "Greece";
			super.definition = "Scheme code for GR (Greece) - Greek RTGS (HERMES).";
		}
	};
	/**
	 * Scheme code for HU (Hungary) - VIBER.
	 */
	public final MMCode codeHungary = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "HUF";
			super.name = "Hungary";
			super.definition = "Scheme code for HU (Hungary) - VIBER.";
		}
	};
	/**
	 * Scheme code for LU (Luxemburg) - Luxembourg RTGS (LIPS).
	 */
	public final MMCode codeLuxemburg = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "LGS";
			super.name = "Luxemburg";
			super.definition = "Scheme code for LU (Luxemburg) - Luxembourg RTGS (LIPS).";
		}
	};
	/**
	 * Scheme code for LV (Latvia).
	 */
	public final MMCode codeLatvia = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "LVL";
			super.name = "Latvia";
			super.definition = "Scheme code for LV (Latvia).";
		}
	};
	/**
	 * Scheme code for ZA (South Africa) - South-African Multiple Option Settlement.
	 */
	public final MMCode codeSouthAfrica = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "MOS";
			super.name = "SouthAfrica";
			super.definition = "Scheme code for ZA (South Africa) - South-African Multiple Option Settlement.";
		}
	};
	/**
	 * Scheme code for MU (Mauritius).
	 */
	public final MMCode codeMauritius = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "MUP";
			super.name = "Mauritius";
			super.definition = "Scheme code for MU (Mauritius).";
		}
	};
	/**
	 * Scheme code for NO (Norway).
	 */
	public final MMCode codeNorway = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "NOC";
			super.name = "Norway";
			super.definition = "Scheme code for NO (Norway).";
		}
	};
	/**
	 * Scheme code for CH (Switzerland).
	 */
	public final MMCode codeSwitzerland = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "PCH";
			super.name = "Switzerland";
			super.definition = "Scheme code for CH (Switzerland).";
		}
	};
	/**
	 * Scheme code for AU (Australia).
	 */
	public final MMCode codeAustralia = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "PDS";
			super.name = "Australia";
			super.definition = "Scheme code for AU (Australia).";
		}
	};
	/**
	 * Scheme code for EG (Egypt).
	 */
	public final MMCode codeEgypt = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "PEG";
			super.name = "Egypt";
			super.definition = "Scheme code for EG (Egypt).";
		}
	};
	/**
	 * Scheme code for FR (France).
	 */
	public final MMCode codeFrancePNS = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "PNS";
			super.name = "FrancePNS";
			super.definition = "Scheme code for FR (France).";
		}
	};
	/**
	 * Scheme code for Ve (Venezuela).
	 */
	public final MMCode codeVenezuela = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "PVE";
			super.name = "Venezuela";
			super.definition = "Scheme code for Ve (Venezuela).";
		}
	};
	/**
	 * Scheme code for SE (Sweden) - Swedish Euro RTGS (SEC).
	 */
	public final MMCode codeSwedenSEC = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "SEC";
			super.name = "SwedenSEC";
			super.definition = "Scheme code for SE (Sweden) - Swedish Euro RTGS (SEC).";
		}
	};
	/**
	 * Scheme code for SI (Slovenia).
	 */
	public final MMCode codeSlovania = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "SIT";
			super.name = "Slovania";
			super.definition = "Scheme code for SI (Slovenia).";
		}
	};
	/**
	 * Scheme code for ES (Spain) - Spanish RTGS (SLBE).
	 */
	public final MMCode codeSpainES = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "SLB";
			super.name = "SpainES";
			super.definition = "Scheme code for ES (Spain) - Spanish RTGS (SLBE).";
		}
	};
	/**
	 * Scheme code for PT (Portugal) - Portuguese RTGS (SPGT).
	 */
	public final MMCode codePortugal = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "SPG";
			super.name = "Portugal";
			super.definition = "Scheme code for PT (Portugal) - Portuguese RTGS (SPGT).";
		}
	};
	/**
	 * Scheme code for SE (Sweden) - SEK RTGS (RIX).
	 */
	public final MMCode codeSwedenSSK = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "SSK";
			super.name = "SwedenSSK";
			super.definition = "Scheme code for SE (Sweden) - SEK RTGS (RIX).";
		}
	};
	/**
	 * Scheme code for FR (France) - French RTGS (TBF).
	 */
	public final MMCode codeFranceFR = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "TBF";
			super.name = "FranceFR";
			super.definition = "Scheme code for FR (France) - French RTGS (TBF).";
		}
	};
	/**
	 * Scheme code for Target.
	 */
	public final MMCode codeTarget = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "TGT";
			super.name = "Target";
			super.definition = "Scheme code for Target.";
		}
	};
	/**
	 * Scheme code for NL (Netherlands) - Dutch RTGS (TOP)
	 */
	public final MMCode codeNetherlands = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "TOP";
			super.name = "Netherlands";
			super.definition = "Scheme code for NL (Netherlands) - Dutch RTGS (TOP)";
		}
	};
	/**
	 * Scheme code for the US national real time gross settlement system.
	 */
	public final MMCode codeFedwire = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "FDW";
			super.name = "Fedwire";
			super.definition = "Scheme code for the US national real time gross settlement system.";
		}
	};
	/**
	 * Scheme code for the Bank of Japan clearing system.
	 */
	public final MMCode codeBankOfJapanNet = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "BOJ";
			super.name = "BankOfJapanNet";
			super.definition = "Scheme code for the Bank of Japan clearing system.";
		}
	};
	/**
	 * Scheme code for the Foreign Exchange Yen Clearing system (FEYCS). It is the Japanese electronic interbank system for sending guaranteed and unconditional yen payments of FX deals for same day settlement from one settlement bank, on behalf of itself or its customers, to another settlement bank.
	 */
	public final MMCode codeForeignExchangeYenClearing = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "FEY";
			super.name = "ForeignExchangeYenClearing";
			super.definition = "Scheme code for the Foreign Exchange Yen Clearing system (FEYCS). It is the Japanese electronic interbank system for sending guaranteed and unconditional yen payments of FX deals for same day settlement from one settlement bank, on behalf of itself or its customers, to another settlement bank.";
		}
	};
	/**
	 * Scheme code for the Zengin system. The electronic payment system for domestic third party transfers managed by the Tokyo Bankers Association.
	 */
	public final MMCode codeZengin = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "ZEN";
			super.name = "Zengin";
			super.definition = "Scheme code for the Zengin system. The electronic payment system for domestic third party transfers managed by the Tokyo Bankers Association.";
		}
	};
	/**
	 * Real Time Gross Settlement System. Payment system that simultaneously clears individual transfers and settles them in central bank money.
	 */
	public final MMCode codeRTGS = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "RTG";
			super.name = "RTGS";
			super.definition = "Real Time Gross Settlement System. Payment system that simultaneously clears individual transfers and settles them in central bank money.";
		}
	};
	/**
	 * Automated Clearing House. Payment system that clears cash transfers and settles the proceeds in a lump sum, usually on a multilateral netting basis.
	 */
	public final MMCode codeACH = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "ACH";
			super.name = "ACH";
			super.definition = "Automated Clearing House. Payment system that clears cash transfers and settles the proceeds in a lump sum, usually on a multilateral netting basis.";
		}
	};
	/**
	 * CHIPS is the Clearing House Interbank Payment System in the US.
	 */
	public final MMCode codeUSChips = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "CHI";
			super.name = "USChips";
			super.definition = "CHIPS is the Clearing House Interbank Payment System in the US.";
		}
	};
	/**
	 * FedNet is a link to a Federal Bank account via the internet.  FedNet enables checking of account balance, transactions, take print outs of account statement, transfer funds to third party accounts, E-shopping, BSNL Payments, Deposit opening, Deposit Renewal, Request for Demand Draft, Cheque Book etc.
	 */
	public final MMCode codeFedNet = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "FDN";
			super.name = "FedNet";
			super.definition = "FedNet is a link to a Federal Bank account via the internet.  FedNet enables checking of account balance, transactions, take print outs of account statement, transfer funds to third party accounts, E-shopping, BSNL Payments, Deposit opening, Deposit Renewal, Request for Demand Draft, Cheque Book etc.";
		}
	};
	/**
	 * Scheme code for DK (Denmark) - Danish Krone RTGS (KRONOS)
	 */
	public final MMCode codeDenmarkDDK = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "DDK";
			super.name = "DenmarkDDK";
			super.definition = "Scheme code for DK (Denmark) - Danish Krone RTGS (KRONOS)";
		}
	};
	/**
	 * Scheme code for AL (Albania) -  Albania Interbank Payment System.
	 */
	public final MMCode codeAlbania = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "AIP";
			super.name = "Albania";
			super.definition = "Scheme code for AL (Albania) -  Albania Interbank Payment System.";
		}
	};
	/**
	 * Scheme code for SE (Sweden) - Sweden BGC Clearing CUG.
	 */
	public final MMCode codeSwedenBGC = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "BCC";
			super.name = "SwedenBGC";
			super.definition = "Scheme code for SE (Sweden) - Sweden BGC Clearing CUG.";
		}
	};
	/**
	 * Scheme code for BB (Barbados) - Barbados RTGS (CBRTGS).
	 */
	public final MMCode codeBarbados = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "BDS";
			super.name = "Barbados";
			super.definition = "Scheme code for BB (Barbados) - Barbados RTGS (CBRTGS).";
		}
	};
	/**
	 * Scheme code for BG (Bulgaria) - Bulgaria RTGS.
	 */
	public final MMCode codeBulgaria = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "BGN";
			super.name = "Bulgaria";
			super.definition = "Scheme code for BG (Bulgaria) - Bulgaria RTGS.";
		}
	};
	/**
	 * Scheme code for BS (Bahamas) - Bahamas RTGS.
	 */
	public final MMCode codeBahamas = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "BHS";
			super.name = "Bahamas";
			super.definition = "Scheme code for BS (Bahamas) - Bahamas RTGS.";
		}
	};
	/**
	 * Scheme code for BW (Botswana) - Botswana Interbank Settlement System.
	 */
	public final MMCode codeBotswana = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "BIS";
			super.name = "Botswana";
			super.definition = "Scheme code for BW (Botswana) - Botswana Interbank Settlement System.";
		}
	};
	/**
	 * Scheme code for PH (Philippines) - Philippines Payment System.
	 */
	public final MMCode codePhilippines = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "BSP";
			super.name = "Philippines";
			super.definition = "Scheme code for PH (Philippines) - Philippines Payment System.";
		}
	};
	/**
	 * Scheme code for ECB (European Central Bank) - ECB Payment Mechanism.
	 */
	public final MMCode codeECB = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "EPM";
			super.name = "ECB";
			super.definition = "Scheme code for ECB (European Central Bank) - ECB Payment Mechanism.";
		}
	};
	/**
	 * Scheme code for the US CHIPS-ACH.
	 */
	public final MMCode codeUSChipsACH = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "EPN";
			super.name = "USChipsACH";
			super.definition = "Scheme code for the US CHIPS-ACH.";
		}
	};
	/**
	 * Scheme code for the US FED-ACH.
	 */
	public final MMCode codeUSFedACH = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "FDA";
			super.name = "USFedACH";
			super.definition = "Scheme code for the US FED-ACH.";
		}
	};
	/**
	 * Scheme code for GH (Ghana) - Ghana Interbank Settlement System (GISS).
	 */
	public final MMCode codeGhana = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "GIS";
			super.name = "Ghana";
			super.definition = "Scheme code for GH (Ghana) - Ghana Interbank Settlement System (GISS).";
		}
	};
	/**
	 * Scheme code for NL (Netherlands) - Netherlands Interpay CUG.
	 */
	public final MMCode codeNetherlandsIP = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "INC";
			super.name = "NetherlandsIP";
			super.definition = "Scheme code for NL (Netherlands) - Netherlands Interpay CUG.";
		}
	};
	/**
	 * Scheme code for JO (Jordan) - Jordan RTGS.
	 */
	public final MMCode codeJordan = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "JOD";
			super.name = "Jordan";
			super.definition = "Scheme code for JO (Jordan) - Jordan RTGS.";
		}
	};
	/**
	 * Scheme code for KE (Kenya) - Kenyan Electronic Payment Settlement System.
	 */
	public final MMCode codeKenya = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "KPS";
			super.name = "Kenya";
			super.definition = "Scheme code for KE (Kenya) - Kenyan Electronic Payment Settlement System.";
		}
	};
	/**
	 * Scheme code for LK (Sri Lanka) - Sri Lanka (Lankasettle).
	 */
	public final MMCode codeSriLanka = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "LKB";
			super.name = "SriLanka";
			super.definition = "Scheme code for LK (Sri Lanka) - Sri Lanka (Lankasettle).";
		}
	};
	/**
	 * Scheme code for SG (Singapore) - Singapore RTGS (MEPS+).
	 */
	public final MMCode codeSingapore = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "MEP";
			super.name = "Singapore";
			super.definition = "Scheme code for SG (Singapore) - Singapore RTGS (MEPS+).";
		}
	};
	/**
	 * Scheme code for MT (Malta) - Malta Realtime Interbank Settlement System.
	 */
	public final MMCode codeMalta = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "MRS";
			super.name = "Malta";
			super.definition = "Scheme code for MT (Malta) - Malta Realtime Interbank Settlement System.";
		}
	};
	/**
	 * Scheme code for NA (Namibian) - Namibian Interbank Settlement System.
	 */
	public final MMCode codeNamibia = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "NAM";
			super.name = "Namibia";
			super.definition = "Scheme code for NA (Namibian) - Namibian Interbank Settlement System.";
		}
	};
	/**
	 * Scheme code for AO (Angola) - Angola RTGS.
	 */
	public final MMCode codeAngola = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "PTR";
			super.name = "Angola";
			super.definition = "Scheme code for AO (Angola) - Angola RTGS.";
		}
	};
	/**
	 * Scheme code for RO (Romania) - Romanian Electronic Payment Operations RT.
	 */
	public final MMCode codeRomaniaEPO = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "ROL";
			super.name = "RomaniaEPO";
			super.definition = "Scheme code for RO (Romania) - Romanian Electronic Payment Operations RT.";
		}
	};
	/**
	 * Scheme code for RO (Romania) - Romanian GSRS.
	 */
	public final MMCode codeRomaniaGSRS = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "ROS";
			super.name = "RomaniaGSRS";
			super.definition = "Scheme code for RO (Romania) - Romanian GSRS.";
		}
	};
	/**
	 * Scheme code for CL (Chile) - Chilean Interbank Payment System.
	 */
	public final MMCode codeChili = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "SCP";
			super.name = "Chili";
			super.definition = "Scheme code for CL (Chile) - Chilean Interbank Payment System.";
		}
	};
	/**
	 * Scheme code for UK (United Kingdom) - CHAPS Sterling RTGS.
	 */
	public final MMCode codeUnitedKingdomGBP = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "STG";
			super.name = "UnitedKingdomGBP";
			super.definition = "Scheme code for UK (United Kingdom) - CHAPS Sterling RTGS.";
		}
	};
	/**
	 * Scheme code for TH (Thailand) - Thailand Payment System (Bahtnet/2).
	 */
	public final MMCode codeThailand = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "THB";
			super.name = "Thailand";
			super.definition = "Scheme code for TH (Thailand) - Thailand Payment System (Bahtnet/2).";
		}
	};
	/**
	 * Scheme code for TZ (Tanzania) - Tanzania Interbank Settlement System (TISS).
	 */
	public final MMCode codeTanzania = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "TIS";
			super.name = "Tanzania";
			super.definition = "Scheme code for TZ (Tanzania) - Tanzania Interbank Settlement System (TISS).";
		}
	};
	/**
	 * Scheme code for TT (Trinidad and Tobago ) - Trinidad and Tobago SAFE-TT.
	 */
	public final MMCode codeTrinidadAndTobago = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "TTD";
			super.name = "TrinidadAndTobago";
			super.definition = "Scheme code for TT (Trinidad and Tobago ) - Trinidad and Tobago SAFE-TT.";
		}
	};
	/**
	 * Scheme code for UG (Uganda) - Uganda National Interbank Settlement System.
	 */
	public final MMCode codeUganda = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "UIS";
			super.name = "Uganda";
			super.definition = "Scheme code for UG (Uganda) - Uganda National Interbank Settlement System.";
		}
	};
	/**
	 * Scheme code for ZW (Zimbabwe) - Zimbabwe Electronic Transfer 	&amp; Settlement System.
	 */
	public final MMCode codeZimbabwe = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "ZET";
			super.name = "Zimbabwe";
			super.definition = "Scheme code for ZW (Zimbabwe) - Zimbabwe Electronic Transfer & Settlement System.";
		}
	};
	/**
	 * Scheme code for ZM (Zambia) - Zambian Interbank Payment 	&amp;Settlement System.
	 */
	public final MMCode codeZambia = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "ZIS";
			super.name = "Zambia";
			super.definition = "Scheme code for ZM (Zambia) - Zambian Interbank Payment &Settlement System.";
		}
	};
	/**
	 * Scheme code for Columbia (CO) - Colombian ACH CENIT Central Bank Payment System.
	 */
	public final MMCode codeColumbia = new MMCode() {
		{
			super.owner_lazy = () -> RTCashClearingSystemCode.repoType();
			super.codeName = "COP";
			super.name = "Columbia";
			super.definition = "Scheme code for Columbia (CO) - Colombian ACH CENIT Central Bank Payment System.";
		}
	};

	public static RTCashClearingSystemCode repoType() {
		repoTypeRef.compareAndSet(null, new RTCashClearingSystemCode());
		return repoTypeRef.get();
	}

	private RTCashClearingSystemCode() {
		super.code = Arrays.asList(codeEBAEuro1Step1, codeAustrian,
				codeNewZealand, codeAzerbaijan, codeBosniaHerzegovina,
				codeBelgium, codeFinland, codeItaly, codeCanada, codeSpainCAM,
				codeIreland, codeUnitedKingdom, codeDenmark,
				codeGermanyRTGSPlus, codeEBAEuro1, codeGermanyELS,
				codeEBAStep1, codeEBAStep2, codeCroatia, codeGreece,
				codeHungary, codeLuxemburg, codeLatvia, codeSouthAfrica,
				codeMauritius, codeNorway, codeSwitzerland, codeAustralia,
				codeEgypt, codeFrancePNS, codeVenezuela, codeSwedenSEC,
				codeSlovania, codeSpainES, codePortugal, codeSwedenSSK,
				codeFranceFR, codeTarget, codeNetherlands, codeFedwire,
				codeBankOfJapanNet, codeForeignExchangeYenClearing, codeZengin,
				codeRTGS, codeACH, codeUSChips, codeFedNet, codeDenmarkDDK,
				codeAlbania, codeSwedenBGC, codeBarbados, codeBulgaria,
				codeBahamas, codeBotswana, codePhilippines, codeECB,
				codeUSChipsACH, codeUSFedACH, codeGhana, codeNetherlandsIP,
				codeJordan, codeKenya, codeSriLanka, codeSingapore, codeMalta,
				codeNamibia, codeAngola, codeRomaniaEPO, codeRomaniaGSRS,
				codeChili, codeUnitedKingdomGBP, codeThailand, codeTanzania,
				codeTrinidadAndTobago, codeUganda, codeZimbabwe, codeZambia,
				codeColumbia);
		super.dataDictionary_lazy = () -> GeneratedRepository.repoType().dataDict;
		super.name = "CashClearingSystemCode";
		super.definition = "Specification of a pre-agreed offering between clearing agents or the channel through which the payment instruction is to be processed.";
		super.example = Arrays.asList("ABE");
	} }